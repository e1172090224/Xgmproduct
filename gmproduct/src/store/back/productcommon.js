...
export default {
    ...
    actions: {
        ...
        DeleteData(context,multipleSelection){
            const selectId = [];
            multipleSelection.filter(item => selectId.push(item.product));
            ajax({
                method: 'post',
                url: '/infocent/projectsearch/projectlist/removebaseinfo',
                data: selectId
            }).then(res => {
        ...
        //添加修改页
        //显示数据
        getFormData({state},product){
            ajax({
                url: '/infocent/projectsearch/projectlist/getbaseinfo',
            }).then(res => {
                const targetData = res.data.data.filter(item => item.product === product);
        ...
        addFormData(context,form){
            ajax({
                url: '/infocent/projectsearch/projectlist/addbaseinfo',
                method: 'post',
                data: form
            }).then(res => {
        ...
        updateFormData(context,form){
            ajax({
                url: '/infocent/projectsearch/projectlist/changebaseinfo',
                method: 'post',
                data: form,
            }).then(res => {
                if (res.data.status === 200) {
